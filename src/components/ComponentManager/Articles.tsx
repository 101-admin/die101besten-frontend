...
const Articles = ({ title, description, slider, ctaButton }: Hotelmomente) => {
  return (
    <section className="w-full max-w-[1440px] mx-auto flex flex-col justify-center items-center py-10 lg:pt-14 lg:pb-32 select-none">
          ...
          >
            <CarouselContent className="flex justify-start items-start w-full gap-4">
              {slider?.map((item, index) => {
                const { ctaButton, description, image, catogory, title } = item;
                return (
                  <CarouselItem
                    key={index}
                  ...
                  >
                    <div className="w-full h-full relative">
                      ...
                      <div className="absolute top-0 left-0 w-full h-full hidden group-hover/show:flex flex-col justify-start items-baseline bg-[#000000B2]">
                        <div className="w-full h-full flex flex-col justify-start items-baseline  pt-32 pl-12 pr-10">
                          <h2 className="w-[113px] h-[24px] bg-gradient-to-r from-[#F49E6E] to-[#B64F32] text-center text-white rounded-lg text-[12px] font-Montserrat font-bold leading-[24px] mb-3 uppercase">
                            {catogory}
                          </h2>
                          <p className=" font-gte font-[350] text-[20px] md:text-[24px] leading-[24px] md:leading-[32px] text-white mb-3">
                            {description?.substring(0, 120)}
                          </p>
                          <Link className="w-full" href={`${ctaButton?.link}`}>
                    ...
                    <h1 className="font-ogg font-normal text-[20px] sm:text-[25px] md:text-[33px] lg:text-[38px] leading-[28px] sm:leading-[33px] md:leading-[38px] lg:leading-[42px] my-3">

...
import {
  getHotelBySlugOptimizedQuery,
  getHotelBasicInfoQuery
} from "@/lib/queries/hotels.optimized.queries";
...
export const HotelsApi = {
  ...
  getCities: unstable_cache(
    ...
    getHotelBySlugPreview: async (slug: string, language = DEFAULT_LANGUAGE) => {
      const slugWithLocale = ensureSlugHasLocaleSuffix(slug, language);
      return previewClient.fetch(getHotelBySlugOptimizedQuery, { slug: slugWithLocale, language, edition: DEFAULT_EDITION }, {
        perspective: "drafts",
        useCdn: false,
        stega: true,
      } as any);
    ...
    /**
     * Fast preview method for basic hotel info only
     */
    getHotelBasicInfoPreview: async (slug: string, language = DEFAULT_LANGUAGE) => {
      const slugWithLocale = ensureSlugHasLocaleSuffix(slug, language);
      return previewClient.fetch(getHotelBasicInfoQuery, { slug: slugWithLocale, language, edition: DEFAULT_EDITION }, {
        perspective: "drafts",
        useCdn: false,
        stega: true,
      } as any);
